...
class LineBotController
{
    ...
    {
        $messgae= \request('text');
        $userId=\request('userId');

        $bot = app('line-bot');
        $textMessageBuilder = new \LINE\LINEBot\MessageBuilder\TextMessageBuilder($messgae);
        $response = $bot->pushMessage( $userId, $textMessageBuilder);
        return redirect('api/line/index')->with('message', '登録したでござる');
    }
    ...
    {
        $friends=LineFriend::get()->toArray();
        return view('line.main.index',compact('friends'));
    }
}